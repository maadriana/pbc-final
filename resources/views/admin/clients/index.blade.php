    ...
    <div class="card-body">
        <form method="GET" class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search by company name or contact..." value="{{ request('search') }}">
            </div>
            <div class="col-md-3">
            ...
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="px-4 py-3">Ongoing Jobs</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <span class="text-muted">{{ $client->user->email }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="text-muted">{{ $client->phone ?: 'Not provided' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="text-center">
                                @if($ongoingJobs > 0)
                                    <span class="badge bg-warning">{{ $ongoingJobs }}</span>
                                @else
                        ...
                        <td class="px-4 py-3">
                            <div class="text-center">
                                @if($completedJobs > 0)
                                    <span class="badge bg-success">{{ $completedJobs }}</span>
                                @else
                                    <span class="text-muted">0</span>
                                @endif
                            </div>
                        ...
                        <td class="px-4 py-3">
                            <div class="btn-group" role="group">
                                ...
                                <a href="{{ route('admin.projects.create', ['client_id' => $client->id]) }}" class="btn btn-success btn-sm" title="Create Job">
                                ...
                                @if(auth()->user()->isSystemAdmin())
                                    <button type="button" class="btn btn-danger btn-sm" onclick="deleteClient({{ $client->id }})" title="Delete Client">
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-5">
                            <div class="text-muted">
                                ...
                                <div class="h5">No clients found</div>
                                <small>
                                    @if(request('search') || request('status') || request('sort'))
                                        Try adjusting your search criteria or <a href="{{ route('admin.clients.index') }}" class="text-decoration-none">clear filters</a>
                                    @else
                                        Clients will appear here when they are created
                                    @endif
                                </small>
                            </div>
                        </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-warning">
                    <small><strong>Warning:</strong> This will also remove all associated projects and PBC requests.</small>
                </div>
            </div>
